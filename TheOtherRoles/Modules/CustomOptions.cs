...
namespace TheOtherRoles {
    public class CustomOption {
        ...
        public static CustomOption Create(int id, CustomOptionType type, string name, bool defaultValue, CustomOption parent = null, bool isHeader = false, Action onChange = null, string heading = "", bool invertedParent = false) {
            return new CustomOption(id, type, name, new string[]{"關", "開"}, defaultValue ? "開" : "關", parent, isHeader, onChange, heading, invertedParent);
        }
    ...
    class LobbyViewSettingsPatch{
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType) {
            ...
            foreach (var option in relevantOptions) {
                ...
                if (option.isHeader && (int)optionType != 99 && option.heading == "" && (option.type == CustomOptionType.Neutral || option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.Modifier)) {
                    viewSettingsInfoPanel.titleText.text = "生成機率";
                }
                if ((int)optionType == 99) {
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "TOR 設定", CustomOptionType.General);
                   // create TOR settings
                createCustomButton(__instance, next++, "RoleOverview", "職業總覽", (CustomOptionType)99);
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", "偽裝者職業", CustomOptionType.Impostor);

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", "獨立職業", CustomOptionType.Neutral);
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", "船員職業", CustomOptionType.Crewmate);
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", "特殊標籤", CustomOptionType.Modifier);

            } else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek) {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", "躲貓貓", CustomOptionType.HideNSeekMain);
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", "躲貓貓", CustomOptionType.HideNSeekRoles);
            } else if (TORMapOptions.gameMode == CustomGamemodes.PropHunt) {
                createCustomButton(__instance, next++, "PropHunt", "物品獵殺", CustomOptionType.PropHunt);
            }
        }
    }
    ...
    class GameOptionsMenuStartPatch {
        ...
        private static void createSettings(GameOptionsMenu menu, List<CustomOption> options) {
            ...
            foreach (CustomOption option in options) {
                ...
                if (option.isHeader && option.heading == "" && (option.type == CustomOptionType.Neutral || option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.Modifier)) {
                    stringOption.TitleText.text = "生成機率";
                }
        ...
        private static void createSettingTabs(GameSettingMenu __instance) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic) {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", "TOR 設定");
    ...
    class GameOptionsDataPatch
    {
        ...
        public static string buildModifierExtras(CustomOption customOption) {
            ...
            if (customOption == CustomOptionHolder.modifierLover) {
                return $" (1 壞: {CustomOptionHolder.modifierLoverImpLoverRate.getSelection() * 10}%)";
            }
            return "";
        }
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly) {
            ...
            foreach (var option in options) {
                if (option.parent == null) {
                    ...
                    if (option.id == 103) //Deputy
                        sb.AppendLine($"- {Helpers.cs(Deputy.color, "警員")}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 224) //Sidekick
                        sb.AppendLine($"- {Helpers.cs(Sidekick.color, "跟班")}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 358) //Prosecutor
                        sb.AppendLine($"- {Helpers.cs(Lawyer.color, "檢察官")}: {option.selections[option.selection].ToString()}");
                }
            }
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            string hudString = counter != 0 && !hideExtras ? Helpers.cs(DateTime.Now.Second % 2 == 0 ? Color.white : Color.red, "(必要時可使用滾輪)\n\n") : "";
